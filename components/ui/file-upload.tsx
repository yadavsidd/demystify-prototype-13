...
export const FileUpload = ({
  ...
  const handleFileChange = (newFiles: File[]) => {
    // Modified: Replace files instead of appending to support single file workflow
    setFiles(newFiles);
          ...
          <div className="relative w-full mt-10 max-w-xl mx-auto">
            {files.length > 0 &&
              files.map((file, idx) => (
                <motion.div
                  key={"file" + idx}
                  layoutId={idx === 0 ? `${id}-thumb` : `${id}-thumb-${idx}`}
                  className={cn(
            ...
            {!files.length && (
              <motion.div
                ...
                className="absolute opacity-0 border border-dashed border-[var(--accent-color)] inset-0 z-30 bg-transparent flex items-center justify-center h-32 mt-4 w-full max-w-[8rem] mx-auto rounded-md"
              ></motion.div>
            )}
          </div>
        </div>
